...
if($origin!==''&&parse_url($origin,PHP_URL_HOST)!==preg_replace('/:\d+$/','',$host))answer(403,'Источник запроса не разрешён.');
...
if(!filter_var($mailTo,FILTER_VALIDATE_EMAIL))answer(503,'Отправка почты пока не настроена.');
...
@touch($rateFile);
$safeName=preg_replace('/[\r\n]+/u',' ',$name);$safePhone=preg_replace('/[\r\n]+/u',' ',$phone);
$body="Имя: {$safeName}\nТелефон: {$safePhone}\nСогласие: получено\nДата: ".date('d.m.Y H:i');
$host=preg_replace('/[^a-z0-9.-]/i','',(string)($_SERVER['HTTP_HOST']??'apostolkov.ru'))?:'apostolkov.ru';
